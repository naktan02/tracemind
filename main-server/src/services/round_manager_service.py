...
class RoundManagerService:
    ...
    def create_training_task(
        ...
        *,
        ...
        return TrainingTask(
            ...
            task_id=task_id or f"task_{round_id}_{uuid4().hex[:8]}",
            round_id=round_id,
            model_id=active_manifest.model_id,
            model_revision=active_manifest.model_revision,
            task_type=task_type,
            training_scope=active_manifest.training_scope,
            ...
            selection_policy=selection_policy or {"max_examples": 128},
    ...
    def publish_next_pair(
        ...
        *,
        ...
        base_state = self._load_base_state(base_manifest)
        next_revision = next_model_revision or f"{base_manifest.model_revision}_next"
        update_payloads = [self._load_update_payload(update) for update in updates]
        ...
        next_manifest = ModelManifest(
            ...
            artifact_kind=base_manifest.artifact_kind,
            artifact_ref=str(artifact_path),
            prototype_version=next_prototype_version,
            training_scope=base_manifest.training_scope,
            training_enabled=base_manifest.training_enabled,
            compatible_task_types=base_manifest.compatible_task_types,
            base_model_id=base_manifest.base_model_id,
            base_model_revision=base_manifest.base_model_revision,
            translation_model_id=base_manifest.translation_model_id,
            translation_model_revision=base_manifest.translation_model_revision,
            notes=f"round_active_pair_only published from {updates[0].round_id}",
        )
