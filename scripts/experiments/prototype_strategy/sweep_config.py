"""Threshold sweep 실험 설정 로딩."""

from __future__ import annotations

from dataclasses import dataclass
from pathlib import Path
from typing import Any

from omegaconf import OmegaConf

from scripts.experiments.prototype_strategy.config import (
    DatasetConfig,
    EmbeddingConfig,
    KMeansConfig,
    DbscanConfig,
    OutputConfig,
    RuntimeConfig,
    _build_embedding_config,
)

PROJECT_ROOT = Path(__file__).resolve().parents[3]
DEFAULT_SWEEP_CONFIG_PATH = (
    PROJECT_ROOT / "scripts/experiments/configs/prototype_threshold_sweep/default.yaml"
)


@dataclass(slots=True, frozen=True)
class ThresholdGridConfig:
    confidence_thresholds: tuple[float, ...]
    margin_thresholds: tuple[float, ...]


@dataclass(slots=True, frozen=True)
class SweepStrategyConfig:
    name: str
    kmeans: KMeansConfig
    dbscan: DbscanConfig


@dataclass(slots=True, frozen=True)
class SweepSelectionPolicyConfig:
    minimum_accepted_ratio: float


@dataclass(slots=True, frozen=True)
class ThresholdSweepConfig:
    dataset: DatasetConfig
    embedding: EmbeddingConfig
    strategy: SweepStrategyConfig
    selection_policy: SweepSelectionPolicyConfig
    threshold_grid: ThresholdGridConfig
    output: OutputConfig
    runtime: RuntimeConfig


def _resolve_path(raw_path: str) -> Path:
    path = Path(raw_path)
    if path.is_absolute():
        return path
    return PROJECT_ROOT / path


def _require_dict(value: Any, *, field_name: str) -> dict[str, Any]:
    if not isinstance(value, dict):
        raise ValueError(f"{field_name} must be a mapping.")
    return value


def _require_list(value: Any, *, field_name: str) -> list[Any]:
    if not isinstance(value, list):
        raise ValueError(f"{field_name} must be a list.")
    return value


def _require_string(payload: dict[str, Any], key: str) -> str:
    value = payload.get(key)
    if not isinstance(value, str) or not value.strip():
        raise ValueError(f"Missing required string field: {key}")
    return value


def load_threshold_sweep_config(
    *,
    config_path: Path | None = None,
    overrides: tuple[str, ...] = (),
) -> ThresholdSweepConfig:
    """YAML 설정과 dotlist override를 병합해 threshold sweep config를 만든다."""
    resolved_path = config_path or DEFAULT_SWEEP_CONFIG_PATH
    yaml_config = OmegaConf.load(resolved_path)
    override_config = OmegaConf.from_dotlist(list(overrides))
    merged = OmegaConf.merge(yaml_config, override_config)
    payload = OmegaConf.to_container(merged, resolve=True)
    if not isinstance(payload, dict):
        raise ValueError("Threshold sweep config root must be a mapping.")

    dataset_payload = _require_dict(payload.get("dataset", {}), field_name="dataset")
    embedding_payload = _require_dict(
        payload.get("embedding", {}),
        field_name="embedding",
    )
    strategy_payload = _require_dict(payload.get("strategy", {}), field_name="strategy")
    kmeans_payload = _require_dict(
        strategy_payload.get("kmeans", {}),
        field_name="strategy.kmeans",
    )
    dbscan_payload = _require_dict(
        strategy_payload.get("dbscan", {}),
        field_name="strategy.dbscan",
    )
    threshold_grid_payload = _require_dict(
        payload.get("threshold_grid", {}),
        field_name="threshold_grid",
    )
    selection_policy_payload = _require_dict(
        payload.get("selection_policy", {}),
        field_name="selection_policy",
    )
    output_payload = _require_dict(payload.get("output", {}), field_name="output")
    runtime_payload = _require_dict(payload.get("runtime", {}), field_name="runtime")

    return ThresholdSweepConfig(
        dataset=DatasetConfig(
            train_jsonl=_resolve_path(_require_string(dataset_payload, "train_jsonl")),
            validation_jsonl=_resolve_path(
                _require_string(dataset_payload, "validation_jsonl")
            ),
            test_jsonl=_resolve_path(_require_string(dataset_payload, "test_jsonl")),
        ),
        embedding=_build_embedding_config(embedding_payload),
        strategy=SweepStrategyConfig(
            name=_require_string(strategy_payload, "name"),
            kmeans=KMeansConfig(
                candidate_ks=tuple(
                    int(value)
                    for value in _require_list(
                        kmeans_payload.get("candidate_ks", []),
                        field_name="strategy.kmeans.candidate_ks",
                    )
                ),
                silhouette_sample_size=int(
                    kmeans_payload.get("silhouette_sample_size", 2000)
                ),
            ),
            dbscan=DbscanConfig(
                eps_values=tuple(
                    float(value)
                    for value in _require_list(
                        dbscan_payload.get("eps_values", []),
                        field_name="strategy.dbscan.eps_values",
                    )
                ),
                min_samples_values=tuple(
                    int(value)
                    for value in _require_list(
                        dbscan_payload.get("min_samples_values", []),
                        field_name="strategy.dbscan.min_samples_values",
                    )
                ),
                search_sample_size=int(dbscan_payload.get("search_sample_size", 3000)),
                min_cluster_coverage=float(
                    dbscan_payload.get("min_cluster_coverage", 0.6)
                ),
            ),
        ),
        selection_policy=SweepSelectionPolicyConfig(
            minimum_accepted_ratio=float(
                selection_policy_payload.get("minimum_accepted_ratio", 0.5)
            )
        ),
        threshold_grid=ThresholdGridConfig(
            confidence_thresholds=tuple(
                float(value)
                for value in _require_list(
                    threshold_grid_payload.get("confidence_thresholds", []),
                    field_name="threshold_grid.confidence_thresholds",
                )
            ),
            margin_thresholds=tuple(
                float(value)
                for value in _require_list(
                    threshold_grid_payload.get("margin_thresholds", []),
                    field_name="threshold_grid.margin_thresholds",
                )
            ),
        ),
        output=OutputConfig(
            output_dir=_resolve_path(_require_string(output_payload, "output_dir"))
        ),
        runtime=RuntimeConfig(
            seed=int(runtime_payload.get("seed", 42)),
        ),
    )
