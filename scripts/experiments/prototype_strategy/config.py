...
from scripts.common.embedding_profiles import resolve_embedding_settings
...
def _build_embedding_config(embedding_payload: dict[str, Any]) -> EmbeddingConfig:
    cache_dir_value = embedding_payload.get("cache_dir")
    resolved = resolve_embedding_settings(
        profile=str(embedding_payload.get("profile", "mxbai")),
        backend=_optional_string(embedding_payload, "backend"),
        model_id=_optional_string(embedding_payload, "model_id"),
        revision=_optional_string(embedding_payload, "revision"),
        batch_size=(
            int(embedding_payload["batch_size"])
            if "batch_size" in embedding_payload
            else None
        ),
        cache_dir=(
            _resolve_path(str(cache_dir_value))
            if cache_dir_value is not None
            else None
        ),
        device=_optional_string(embedding_payload, "device"),
        task_prefix=(
            str(embedding_payload["task_prefix"])
            if "task_prefix" in embedding_payload
            else None
        ),
        local_files_only=(
            bool(embedding_payload["local_files_only"])
            if "local_files_only" in embedding_payload
            else None
        ),
        hash_dim=(
            int(embedding_payload["hash_dim"])
            if "hash_dim" in embedding_payload
            else None
        ),
    )
    return EmbeddingConfig(
        ...
        task_prefix=resolved.task_prefix,
        local_files_only=resolved.local_files_only,
        hash_dim=resolved.hash_dim,
    )
...
def _build_config(payload: dict[str, Any]) -> ExperimentConfig:
    ...
    return ExperimentConfig(
        ...
        thresholds=ThresholdConfig(
            confidence_threshold=float(
                thresholds_payload.get("confidence_threshold", 0.6)
            ),
            margin_threshold=float(thresholds_payload.get("margin_threshold", 0.02)),
        ),
        projection=ProjectionConfig(
