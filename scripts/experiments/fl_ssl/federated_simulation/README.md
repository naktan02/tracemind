...
- `flow_models.py`
  - FL simulation 내부 active state와 round execution context
- `simulation.py`
  - public `run_simulation_request`와 legacy `run_simulation` wrapper
- `bootstrap.py`
  - FL simulation bootstrap 단계
- `round_loop.py`
  - client/server round loop 단계
- `result_builder.py`
  - final result/report 조립 단계
- `runtime.py`
  - `main_server`/`agent` 코어 객체를 experiment용 저장소와 경로로 조립
- `method_runtime.py`
  - FL SSL descriptor를 기본 simulation runtime adapter로 변환
- `task_config.py`
...
- method descriptor source of truth는 `methods/federated_ssl/`이다.
  이 package의 `method_runtime.py`는 descriptor를 simulation runtime으로 연결하는
  adapter만 둔다.
