...
이 폴더는 중앙집중형 PEFT encoder classifier control 실행 entrypoint만 둔다.
...
기본 `output_dir`는
`runs/train_peft_ssl_classifier/consistency/labeled-..._unlabeled-..._validation-..._test-...`
형태로 설정된다. 주소록은
`conf/execution_context/query_data_source/default.yaml`이 소유한다.
`run_controls/central_ssl/budget=smoke`를 선택하면 같은 하위 구조를
`runs/_smoke/train_peft_ssl_classifier/...` 아래에 만든다.
...
```text
output_dir=runs/train_peft_ssl_classifier/consistency/<selection_slug>/<method_name>/<run_id>
adapter_dir=...
classifier_path=...
manifest=...
report_json=...
