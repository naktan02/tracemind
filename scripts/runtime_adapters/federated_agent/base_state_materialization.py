...
def load_lora_classifier_base_partition_parameters(
    *,
    ...
    """server-published LoRA-classifier partitioned base state를 materialize한다."""

    if round_base_snapshot_cache is None:
        return _materialize_lora_classifier_base_partition_parameters(
            active_adapter_state=active_adapter_state,
            output_dir=output_dir,
            aggregated_at=aggregated_at,
        )
    snapshot = round_base_snapshot_cache.get_or_materialize(
        key=_lora_classifier_partition_cache_key(active_adapter_state),
        materialize=lambda: _materialize_lora_classifier_base_partition_parameters(
            active_adapter_state=active_adapter_state,
            output_dir=output_dir,
            aggregated_at=aggregated_at,
        ),
...
def _lora_classifier_partition_cache_key(
    ...
    return RoundBaseSnapshotCacheKey(
        ...
        schema_version=str(active_adapter_state.schema_version),
        artifact_refs=lora_classifier_base_state_artifact_refs(active_adapter_state),
        materializer_name=LORA_CLASSIFIER_BASE_PARTITIONED_STATE_MATERIALIZER_NAME,
    )
