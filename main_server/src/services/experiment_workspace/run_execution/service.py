...
from main_server.src.services.experiment_workspace.run_execution.result_summary import (
...
class ExperimentRunService:
    ...
    process_launcher: runtime_support.ProcessLauncher = field(
        default_factory=lambda: runtime_support.launch_local_process
    )
    _active_processes: dict[str, runtime_support.LocalExperimentProcessHandle] = field(
    ...
    def launch_run(
        ...
        run_id = runtime_support.build_experiment_run_id(created_at=now)
        run_dir = self.run_repository.run_dir(run_id)
        run_dir.mkdir(parents=True, exist_ok=True)
        run_paths = runtime_support.build_experiment_run_paths(run_dir)
        ...
        record = runtime_support.build_running_run_record(
            run_id=run_id,
    ...
    def _refresh_record(
        ...
        updated_record = runtime_support.build_finished_run_record(
            record,
