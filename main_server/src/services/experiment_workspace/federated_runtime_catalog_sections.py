"""연합 런타임 track용 custom catalog section builder."""
...
def build_training_algorithm_profile_section(
    ...
    """학습 알고리즘 profile config group을 catalog section으로 직렬화한다."""

    items: list[CatalogItemPayload] = []
    for path in context.iter_yaml_files("scripts/conf/fl_ssl/algorithm_profile"):
        ...
        runtime_paths = resolve_training_profile_runtime_paths(
    ...
    return CatalogSectionPayload(
        ...
        source_of_truth="scripts/conf/fl_ssl/algorithm_profile",
        source_kind="hydra_config_group",
        items=tuple(items),
    )


def resolve_training_profile_runtime_paths(
    objective_config: TrainingObjectiveConfig,
) -> tuple[str, ...]:
    """Agent runtime import 없이 objective 조합의 catalog compatibility를 계산한다."""

    runtime_paths = [FEDERATED_SIMULATION_RUNTIME_PATH]
    if not is_local_training_runtime_catalog_compatible(objective_config):
...
def is_local_training_runtime_catalog_compatible(
    objective_config: TrainingObjectiveConfig,
) -> bool:
...
def build_adapter_family_catalog_section(
    ...
    """shared adapter family metadata section을 조립한다."""
...
def build_aggregation_backend_section(
    ...
    """aggregation backend registry section을 조립한다."""
...
def build_training_backend_section(
    ...
    """training backend registry section을 조립한다."""
...
def build_training_example_backend_section(
    ...
    """example generation backend registry section을 조립한다."""
...
def build_evidence_backend_section(
    ...
    """evidence backend registry section을 조립한다."""
...
def build_scoring_backend_section(
    ...
    """scoring backend registry section을 조립한다."""
...
def build_acceptance_policy_section(
    ...
    """acceptance policy registry section을 조립한다."""
...
def build_privacy_guard_section(
    ...
    """privacy guard registry section을 조립한다."""
