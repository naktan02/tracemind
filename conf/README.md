...
- `strategy_axes/trainable_state/update_family`, `round_runtime.aggregation_backend_name`
  ...
  - `strategy_axes/trainable_state/update_family/*`가 `update_family_name`과
    ...
    `validation_evaluator`를 함께 제공한다. v1 shared payload가 필요한 family는
    `payload_adapter_kind`와 `adapter_family_name` compatibility alias도 이 leaf에서
    함께 선언한다.
  ...
  - v1 payload compatibility 때문에 composed runtime에는
    `adapter_family_name=peft_classifier` alias도 남아 있지만, root entrypoint가
    직접 고르지 않는다. 새 실행 조합과 산출물 slug는
    `update_family_name=peft_text_classifier`를 기준으로 해석한다.
