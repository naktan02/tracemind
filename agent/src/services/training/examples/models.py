...
class EmbeddedTrainingExample:
    ...
    weak_scored_event: ScoredEvent | None = None
    weak_embedding: list[float] | None = None
    strong_scored_event: ScoredEvent | None = None
    strong_embedding: list[float] | None = None
    strong_base_embedding: list[float] | None = None
    metadata: dict[str, str | int | float | bool] = field(default_factory=dict)

    @property
    def evidence_scored_event(self) -> ScoredEvent:
        """Pseudo-label evidence를 계산할 기준 view."""

        return self.weak_scored_event or self.scored_event

    @property
    def update_scored_event(self) -> ScoredEvent:
        """학습/update를 적용할 기준 view."""

        return self.strong_scored_event or self.scored_event
