...
- `fedavg_projection.py`: LoRA-classifier state/update payload를 FedAvg core 입력으로
  변환하고 logical artifact slot을 server context에 요청
...
- FL runtime state/update payload shape는 `shared`, update upload와 artifact 저장은
  `agent`/`main_server`, aggregation method 산술은 `methods/federated/aggregation/`이
  맡는다.
