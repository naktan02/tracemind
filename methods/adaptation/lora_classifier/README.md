...
- `federated_ssl/`: FedMatch 같은 FL SSL method-local objective를 LoRA-classifier
  model/loaders, partitioned delta, update payload로 실행하는 adapter-family slice.
  method 의미는 `methods/federated_ssl/<method>/`가 계속 소유한다.
