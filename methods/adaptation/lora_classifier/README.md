...
- `aggregation/partitioned_fedavg.py`: shared update의 `partitioned_deltas`를 client별로
  먼저 병합한 뒤 LoRA-classifier FedAvg 산술과 state projection을 재사용한다.
...
- `partitioned_fedavg.py`는 별도 method-specific runtime이 아니라 같은 family의 server
  update 해석 backend다. `sigma/psi`처럼 왜 partition이 생겼는지는
