...
method descriptor YAML에는 `scenario`, `use_original_parameters`,
`parameter_overrides` 같은 실행 선택값만 둔다. trace/report metadata는
`methods/federated_ssl/<method>/descriptor.py`에서 파생한다.
