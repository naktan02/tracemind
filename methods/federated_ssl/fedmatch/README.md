...
- 공통 `peer_context=fixed_probe_output_knn` simulation adapter에서 FedMatch
  `effective_parameters`의 `num_helpers`/`helper_refresh_interval`을 읽어 KDTree
  우선 helper client context를 만들고 method-owned LoRA trainer까지 전달하는 seam
...
probability provider와 `labels-at-server` client-local `psi` upload slice는
simulation에서 열렸지만 sparse S2C/C2S delta sync, labels-at-server supervised
server step은 다음 단계 capability로 남긴다.
