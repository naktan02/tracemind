...
- FedAvg 같은 reusable backend 가중 평균 산술
- 빈 update, total weight 0, key/dimension mismatch 같은 backend-level validation
- aggregation backend strategy wiring
- method metadata registration은 payload adapter projection registration이 제공하고,
...
Payload adapter별 FedAvg core, delta 해석, payload projection은
`methods/adaptation/<family>/`가 소유한다.
partitioned delta payload를 materialize하고 평균하는 일은 payload adapter가 소유한다.
FedMatch의 `sigma/psi` 같은 scheme 의미는 `methods/federated_ssl/<method>/`가
소유한다.
