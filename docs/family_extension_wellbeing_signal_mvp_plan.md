...
- provider, prompt, safety policy, response plan/validation, conversation 저장은 agent runtime이 소유한다.
- LLM은 agent가 만든 response plan의 required move를 실행하는 역할이며, safety 단계별 검증에 실패하면 `local_guarded` fallback으로 돌아간다.
