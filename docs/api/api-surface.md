...
| POST | `/api/v1/captured-text/debug-job/run-view-generation` | pending captured text weak/strong view만 즉시 생성 | `agent/src/api/captured_text.py` |
| POST | `/api/v1/captured-text/debug-job/run-analysis` | generated weak text를 agent-local inference pipeline에 넣어 분석만 즉시 실행 | `agent/src/api/captured_text.py` |
...
agent FL active cache를 직접 읽지 않는다. debug job은 view generation과 analysis
실행을 분리한다. `run-view-generation`은 weak/strong view만 만들고,
`run-analysis`가 generated weak text를 agent-local inference pipeline에 넣어
`analysis_events`, `analysis_category_scores`에 분류 결과를 저장한다.
captured text 학습 입력은 generated weak/strong view source에서 시작한다.
...
| GET | `/api/v1/runtime-profile/status` | agent-local active runtime profile 상태 조회 | `agent/src/api/runtime_profile.py` |
| POST | `/api/v1/runtime-profile/sync` | main server current runtime profile 검증/동기화 | `agent/src/api/runtime_profile.py` |

runtime profile sync는 상시 분석 runtime 선택값을 동기화한다. active profile이
서버와 다르면 shared adapter current state도 함께 pull한 뒤
`agent_runtime_profiles`에 active profile로 저장한다. 날짜 필드는 표시/감사용이며,
최신성 비교는 `profile_id`, `profile_revision`, `payload_checksum`만 사용한다.
...
`agent/src/features/training_runtime/current_task/runner.py`가
소유한다.
...
| GET | `/api/v1/child-support/proactive-prompt` | 아이 화면 진입 시 선제 발화 필요 여부를 side effect 없이 조회 | `agent/src/api/child_support.py` |
| POST | `/api/v1/child-support/proactive-prompt/claim` | 실제 표시 직전 선제 발화를 agent-local 첫 assistant turn으로 claim | `agent/src/api/child_support.py` |
...
| `ChildSupportProactivePromptClaimRequestPayload` | `agent/src/contracts/child_support_contracts.py` |
...
proactive prompt GET은 polling/status 조회만 수행한다. conversation/message 저장과
LLM proactive 문구 생성은 content tab에 실제 표시할 수 있을 때 claim route에서만
수행한다.
...
| Method | Path | 역할 | Source |
|---|---|---|---|
| GET | `/api/v1/agent-runtime-profile/current` | 서버 current agent runtime profile 조회 | `main_server/src/api/agent_runtime_profile.py` |
| POST | `/api/v1/agent-runtime-profile/validate` | agent-local profile 최신성 확인 | `main_server/src/api/agent_runtime_profile.py` |
| POST | `/api/v1/admin/runtime-profile` | 서버 current agent runtime profile 수동 전환 | `main_server/src/api/admin.py` |
| GET | `/api/v1/admin/runtime-profile/current` | 관리자용 current runtime profile 조회 | `main_server/src/api/admin.py` |

이 API는 SSL/FSSL training method 전환 API와 별개다. training method 전환은 round
task와 local update objective를 정하고, runtime profile은 captured text/typing
segment/wellbeing 분석에 쓰는 scorer, embedding, adapter mechanism, required state
kind를 정한다. profile이 서버에 명시 저장되어 있지 않으면 main server는 active
manifest와 `TRACEMIND_AGENT_RUNTIME_*` 환경값으로 current profile을 구성할 수 있다.
