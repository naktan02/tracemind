...
- LocalUpdateProfile, method descriptor, payload family contract, round runtime profile을
  함께 검증한다.
...
4. compatibility validator를 최소 구현한다. method descriptor, payload family contract,
...
aggregation method는 `methods/federated/aggregation/`, adapter-family projection은 각
family-owned `methods/adaptation/<family>/fedavg_projection.py`로 옮겼다.
`main_server` aggregation package에는 generic executor/registry와 server-owned artifact
ref capability만 남긴다. `assets/prototypes`는 catch-all assets package에서
...
- adapter-family projection은 `methods/adaptation/<family>/fedavg_projection.py` 같은
  family-owned module에 둔다.
...
- 좋은 상태: aggregation registry는 primitive와 implementation-local registration으로
  낮아졌다.
- 좋은 상태: aggregation method/projection은 `methods/federated/aggregation/`이 소유하고,
  main_server는 generic executor와 server-owned artifact ref capability만 둔다.
