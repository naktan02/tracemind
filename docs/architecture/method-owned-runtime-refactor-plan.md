...
2. Batch 3 profile/default 작업은 완료됐다. Batch 4-B는 진행 중이다. 1차로
   server-owned LoRA artifact-ref materializer/loader를 붙였고, 다음은 server
   policy executor와 round state exchange capability seam이다.
...
- 진행 중: live server runtime의 method descriptor 기반 `server_policy_executor`는
  기본 round aggregation backend policy까지만 1차 연결했다. `round_state_exchange`
  capability는 아직 없으므로, 복잡한 FL method를 붙일 때 이 seam 없이 round lifecycle에
  method-specific 분기가 들어가면 구조가 다시 얕아진다.
