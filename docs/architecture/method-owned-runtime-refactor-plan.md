...
상태: 1차 완료. `LocalTrainingService`에서 update 생성, privacy protection,
...
- 완료: privacy guard를 `agent/src/services/training/execution/privacy_guards/`
  package로 분리했다. guard registry는 primitive만 맡고 concrete guard는 구현 옆
  decorator로 등록한다.
- 완료: diagonal-scale heuristic과 LoRA-classifier concrete training backend를
  ...
  `training/backends/training` package는 methods-owned registry facade만 남긴다.
...
상태: 현재 WIP 다음에 이어갈 profile 작업. Batch 번호는 계획 식별자일 뿐이고, 실제
다음 단계는 아래 handoff 순서를 따른다.
...
1. 현재 작업트리의 main_server aggregation/prototype 정리를 먼저 커밋한다.
   이 변경은 Batch 4-A 성격이지만, 이미 진행된 구조 정리이므로 먼저 닫아 regression
   추적 단위를 분리한다.
2. 그 다음 profile source-of-truth 작업으로 이동한다. 목표는 실행값을 Hydra에 모으고,
...
1. Hydra profile과 Python mapping/default를 inventory한다.
   ...
   `methods/federated_ssl/training_default_values.py`,
   `methods/federated_ssl/training_defaults.py`다.
...
3. profile resolve output을 typed object로 정의한다. 이 object는 runtime 실행값을 새로
   만들지 않고 Hydra config와 descriptor metadata를 검증한 결과만 담는다.
4. compatibility validator를 최소 구현한다. method descriptor, adapter family metadata,
   local update profile, round runtime profile, runtime capability를 함께 확인한다.
5. Python default/mapping module을 legacy facade 또는 fixture helper로 낮춘다. 삭제가
...
상태: 진행 중. round family concrete module은 generic runtime으로 접었고,
aggregation adapter도 adapter family 파일이 아니라 `fedavg.py` 같은 aggregation
method 파일 중심으로 정리했다. `fedavg.py` 내부도 family별 service/config class 누적
대신 generic `FedAvgAggregationRuntime`과 adapter-family payload spec으로 접었다.
`assets/prototypes`는 catch-all assets package에서 `federation/prototypes`
server-owned artifact lifecycle package로 좁혔다.
...
- `main_server/src/services/federation/rounds/families/registry.py`를 shared adapter
  family metadata 기반 generic runtime으로 유지한다.
- aggregation 파일은 `fedavg.py` 같은 method 단위 runtime adapter로 둔다. adapter
  family별 파일이나 family별 aggregation service/config class를 추가하지 않는다.
- aggregation math는 `methods/federated/aggregation/`에 둔다.
...
- 좋은 상태: aggregation registry는 primitive와 implementation-local registration으로
  낮아졌다.
- 좋은 상태: aggregation adapter는 `fedavg.py` method file 중심으로 정리되었고,
  methods aggregation core를 호출한다.
- 닫힘: `rounds/families/registry.py`는 concrete family 파일을 import하지 않고,
  shared adapter family metadata와 aggregation backend를 generic round family runtime으로
  조합한다.
- 위험: 실제 LoRA artifact-ref materializer가 붙을 때 storage/repository 정책이
  aggregation method adapter로 새면 안 된다.
