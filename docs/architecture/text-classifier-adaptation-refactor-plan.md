...
`peft_text_classifier`, `classification/linear_head`, `peft_adapters`로
...
3. modality-independent classifier-head 구현은 `methods/classification/linear_head/`
   아래로 두고, text encoder 관련 구현은 `methods/adaptation/peft_text_classifier/`
   아래로 모은다.
...
5. FedAvg algorithm은 `methods/federated/aggregation/fedavg/`가 소유하고,
   classification/peft_text_classifier aggregation 계층은 FedAvg 입력/출력 projection만
   소유한다.
...
`classification/aggregation/*`와 `peft_text_classifier/aggregation/*`는 family projection
...
| 현재 위치 | 목표 위치 | 비고 |
|---|---|---|
| `methods/adaptation/lora_classifier/config.py` | `methods/adaptation/peft_text_classifier/config.py` | LoRA 전용값은 PEFT adapter config로 분리 |
| `methods/adaptation/lora_classifier/training/modeling.py` | `methods/adaptation/peft_text_classifier/modeling.py` | text classifier + encoder adapter composition |
| `methods/adaptation/lora_classifier/training/*` | `methods/adaptation/peft_text_classifier/training/*` | local training primitive |
| `methods/adaptation/lora_classifier/update/*` | `methods/adaptation/peft_text_classifier/update/*` | local update payload/materialization |
| `methods/adaptation/lora_classifier/aggregation/fedavg.py` | `methods/adaptation/peft_text_classifier/aggregation/peft_encoder_fedavg_projection.py` | FedAvg algorithm이 아니라 PEFT encoder projection |
| `methods/adaptation/lora_classifier/aggregation/partitioned_delta_average.py` | `methods/adaptation/peft_text_classifier/aggregation/peft_encoder_partitioned_projection.py` | sigma/psi projection |
| `methods/adaptation/lora_classifier/aggregation/state_projection.py` | `methods/adaptation/peft_text_classifier/aggregation/peft_encoder_state_projection.py` | canonical tensor state projection |
| `methods/adaptation/lora_classifier/federated_ssl/partitioned_*.py` | `methods/adaptation/peft_text_classifier/federated_ssl/partitioned/*.py` | method-neutral partitioned primitive |
...
- architecture guard 후보를 정한다.
  `methods/adaptation/peft_text_classifier/**`는 `methods.federated_ssl.fedmatch`를
  import하지 않는다.
- `methods/adaptation/peft_text_classifier/**` 새 내부 코드는
  `methods.adaptation.lora_classifier`를 import하지 않는다.
- `methods/adaptation/peft_text_classifier/aggregation/**`는 weighted average policy를
  직접 구현하지 않고 generic FedAvg core projection만 소유한다.
- `methods/classification/linear_head/**`는 `methods.adaptation.peft_text_classifier`와
...
- `classification/`, `classification/linear_head/`,
  `classification/aggregation/`, `text_classifier/`, `peft_text_classifier/`,
  `peft_text_classifier/aggregation/`,
...
`peft_text_classifier/aggregation/peft_encoder_fedavg_projection.py`,
...
`peft_text_classifier`로 해석한다. 각 dispatcher는 family별 path override를
직접 소유하지 않는다.
