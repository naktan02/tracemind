...
`peft_text_encoder`, `classification/linear_head`, `peft_adapters`로
...
2. `classifier_head/`와 `peft_text_encoder/`도 sibling으로 키우지 않는다.
   classifier-head 자체는 modality-independent classification primitive이고,
   PEFT text encoder는 text-specific adaptation variant다.
3. modality-independent classifier-head 구현은 `methods/classification/linear_head/`
   아래로 두고, text encoder 관련 구현은 `methods/adaptation/peft_text_encoder/`
   아래로 모은다.
...
5. FedAvg algorithm은 `methods/federated/aggregation/fedavg/`가 소유하고,
   classification/peft_text_encoder aggregation 계층은 FedAvg 입력/출력 projection만
   소유한다.
...
methods/adaptation/
  ...
  classification/
    ...
    linear_head/
      ...
      aggregation/
        README.md
        linear_head_fedavg_projection.py
...
`classification/aggregation/*`와 `peft_text_encoder/aggregation/*`는 family projection
...
| 현재 위치 | 목표 위치 | 비고 |
|---|---|---|
| `methods/adaptation/lora_classifier/config.py` | `methods/adaptation/peft_text_encoder/config.py` | LoRA 전용값은 PEFT adapter config로 분리 |
| `methods/adaptation/lora_classifier/training/modeling.py` | `methods/adaptation/peft_text_encoder/modeling.py` | text classifier + encoder adapter composition |
| `methods/adaptation/lora_classifier/training/*` | `methods/adaptation/peft_text_encoder/training/*` | local training primitive |
| `methods/adaptation/lora_classifier/update/*` | `methods/adaptation/peft_text_encoder/update/*` | local update payload/materialization |
| `methods/adaptation/lora_classifier/aggregation/fedavg.py` | `methods/adaptation/peft_text_encoder/aggregation/peft_encoder_fedavg_projection.py` | FedAvg algorithm이 아니라 PEFT encoder projection |
| `methods/adaptation/lora_classifier/aggregation/partitioned_delta_average.py` | `methods/adaptation/peft_text_encoder/aggregation/peft_encoder_partitioned_projection.py` | sigma/psi projection |
| `methods/adaptation/lora_classifier/aggregation/state_projection.py` | `methods/adaptation/peft_text_encoder/aggregation/peft_encoder_state_projection.py` | canonical tensor state projection |
| `methods/adaptation/lora_classifier/federated_ssl/partitioned_*.py` | `methods/adaptation/peft_text_encoder/federated_ssl/partitioned/*.py` | method-neutral partitioned primitive |
| `methods/adaptation/classifier_head/*` | `methods/classification/linear_head/*` | modality-independent feature-head variant |
| `methods/adaptation/classifier_head/aggregation/fedavg.py` | `methods/classification/linear_head/aggregation/linear_head_fedavg_projection.py` | feature head projection |
...
- architecture guard 후보를 정한다.
  `methods/adaptation/peft_text_encoder/**`는 `methods.federated_ssl.fedmatch`를
  import하지 않는다.
- `methods/adaptation/peft_text_encoder/**` 새 내부 코드는
  `methods.adaptation.lora_classifier`를 import하지 않는다.
- `methods/adaptation/peft_text_encoder/aggregation/**`는 weighted average policy를
  직접 구현하지 않고 generic FedAvg core projection만 소유한다.
- `methods/classification/linear_head/**`는 `methods.adaptation.peft_text_encoder`와
...
- `classification/`, `classification/linear_head/`,
  `classification/aggregation/`, `text_classifier/`, `peft_text_encoder/`,
  `peft_text_encoder/aggregation/`,
...
- `lora_classifier/config.py`, `training/*`, `update/*`,
  ...
  `runtime_compatibility.py`를 `peft_text_encoder/` 아래로 이동한다.
...
`peft_text_encoder/`로 이동했다. 기존 `lora_classifier` methods 경로는 삭제했고,
...
- 2단계에서 FedMatch 직접 의존이 제거된 partitioned 파일만
  `peft_text_encoder/federated_ssl/partitioned/`로 이동한다.
...
`partitioned_training_loop.py`를 `peft_text_encoder/federated_ssl/partitioned/`
...
- `classifier_head/`를 `classification/linear_head/`로 이동한다.
...
`classification/linear_head/`로 이동했고, `aggregation/fedavg.py`는
`classification/linear_head/aggregation/linear_head_fedavg_projection.py`로 이동했다.
이전 단계의 legacy `text_classifier/feature_head`와 기존 `classifier_head` direct path는
...
`peft_text_encoder/aggregation/peft_encoder_fedavg_projection.py`,
...
`peft_text_encoder/federated_ssl/`로 이동했고 legacy path는 삭제했다.
...
`peft_text_encoder/aggregation/linear_head_fedavg_projection.py` legacy shim은 제거했고
...
`methods/adaptation/payload_adapter_modules.py`에서 `peft_classifier` 구현 root를
`peft_text_encoder`로 해석한다. 각 dispatcher는 family별 path override를
직접 소유하지 않는다.
