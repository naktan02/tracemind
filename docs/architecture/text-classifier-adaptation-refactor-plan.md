...
2. `classifier_head/`와 `peft_text_classifier/`도 sibling으로 두지 않는다.
   둘 다 text classifier family 아래의 adaptation variant다.
3. text classifier 관련 구현은 `methods/adaptation/text_classifier/` 아래로 모은다.
...
5. FedAvg algorithm은 `methods/federated/aggregation/fedavg/`가 소유하고,
   `text_classifier/aggregation/`은 FedAvg 입력/출력 projection만 소유한다.
...
`text_classifier/aggregation/*`는 family projection 계층이다. 여기서 weighted average,
client weighting, global aggregation policy를 새로 구현하지 않는다. 그런 algorithm은
`methods/federated/aggregation/` 아래 generic aggregation core가 소유한다.
...
| `methods/adaptation/classifier_head/*` | `methods/adaptation/text_classifier/feature_head/*` | text classifier의 feature-head variant |
| `methods/adaptation/classifier_head/aggregation/fedavg.py` | `methods/adaptation/text_classifier/aggregation/feature_head_fedavg_projection.py` | feature head projection |
...
- `text_classifier/`, `text_classifier/peft_encoder/`,
  `text_classifier/feature_head/`, `text_classifier/aggregation/`,
