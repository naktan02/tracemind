...
`peft_text_classifier`, `classification/feature_head`, `peft_adapters`로
...
- `methods/adaptation/classification/**`는 `methods.adaptation.peft_text_classifier`와
...
`peft_text_classifier/aggregation/feature_head_fedavg_projection.py` legacy shim은 제거했고
