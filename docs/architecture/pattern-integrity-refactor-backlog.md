...
2. Batch 4-B: LoRA artifact-ref materializer와 server policy executor capability를
   추가할 때 aggregation/round lifecycle로 method 의미가 새지 않게 한다.
3. Batch 5: SSL hook bundle 교체성, prototype score policy package 분리,
...
- FedMatch/FedLGMatch/(FL)^2 실제 학습 구현을 시작하지 않는다. 단, method 폴더 중심
  recipe/extension rule은 문서와 validator에 반영한다.
