...
- `methods/adaptation/<family>/fedavg_projection.py`: adapter family별 payload projection과
  다음 state materialization을 family-owned module로 이동했다.
...
1. 현재 WIP를 먼저 커밋한다. 포함 범위는 main_server aggregation ownership correction,
...
- compatibility validator는 method descriptor, payload family contract,
...
- `main_server/src/services/federation/rounds/`
  ...
  - 진행: adapter family별 payload projection은
    `methods/adaptation/<family>/fedavg_projection.py`로 이동했다.
...
- `main_server/src/services/experiment_workspace/catalog/local_training_registry_snapshot.py`
  ...
  - 방향: agent runtime registry wiring은 구현 옆 catalog entry를 사용한다. main_server
    ...
    여기에 누적하지 않는다.
- `methods/federated_ssl/training_default_values.py`, `training_defaults.py`
  - 패턴: Profile / Defaults
  ...
  - 남은 문제: `training_default_values.py`와 `training_defaults.py`는 production/API
    fallback과 테스트 fixture 역할이 섞여 있다.
  - 다음 작업: production fallback default와 experiment profile source-of-truth를
    분리하고, Python default module은 compatibility facade 또는 fixture helper로
    좁힌다.
