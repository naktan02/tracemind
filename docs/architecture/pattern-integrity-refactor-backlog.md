...
- `agent/src/services/training/execution/privacy_guards/`: privacy guard Protocol,
  ...
  등록한다.
- `main_server/src/services/federation/rounds/aggregation/runtime_adapter.py`: aggregation
  adapter마다 반복하던 base state 타입, adapter_kind, update lineage, update 타입
  검증을 공통 runtime-boundary helper로 올렸다. family별 aggregation 파일은 methods
  FedAvg core 입력 변환과 다음 state materialization에 집중한다.
- `main_server/src/services/federation/rounds/aggregation/artifact_refs.py`: LoRA-classifier
  aggregation service 안에 있던 server-owned aggregate artifact ref 생성 규칙을
  artifact-ref capability helper로 분리했다. 실제 artifact materializer가 붙을 때는
  이 seam 뒤에 저장/해석 구현을 연결한다.
- `main_server/src/services/federation/rounds/aggregation/`: adapter family별
  `diagonal_scale.py`, `classifier_head.py`, `lora_classifier.py` module을 제거하고
  `fedavg.py` aggregation method runtime adapter로 합쳤다. `fedavg.py` 내부도
  family별 service/config class 누적 대신 generic runtime과 family payload spec으로
  접었다. 새 adapter family 추가가 main_server aggregation package의 family 파일이나
  family별 aggregation class 증가로 이어지지 않게 architecture guard를 추가했다.
...
현재 작업 흐름은 Batch 4-A 성격의 main_server family/aggregation/prototype 위치 정리가
profile 작업보다 먼저 진행된 상태다. 다음 작업자는 Batch 번호보다 아래 순서를 우선한다.

1. 현재 WIP를 먼저 커밋한다. 포함 범위는 main_server aggregation method-file 정리,
   `federation/prototypes` 이동, 관련 README/test/architecture guard다.
2. Profile source-of-truth와 compatibility validator를 정리한다. 실행값은 Hydra에 두고,
   Python은 resolver/validator/legacy facade/test fixture helper로 좁힌다.
3. Batch 4-B: LoRA artifact-ref materializer와 server policy executor capability를
   추가할 때 aggregation/round lifecycle로 method 의미가 새지 않게 한다.
4. Batch 5: SSL hook bundle 교체성, prototype score policy package 분리,
   classifier training template 공통화를 진행한다.
5. Batch 6: scripts runtime adapter, artifact writer/exporter, diagnostics serializer를
   정리한다.
6. Batch 7: dummy method dry-run과 extension guard로 새 method 추가 비용을 검증한다.
...
- 먼저 현재 main_server aggregation/prototype WIP를 커밋한다. 다음 profile 변경과 섞이면
  regression 원인을 추적하기 어렵다.
...
- compatibility validator는 method descriptor, adapter family metadata,
  LocalUpdateProfile, round runtime profile, runtime capability를 한 번에 본다.
- 실패는 training 중간이 아니라 profile resolve/bootstrap에서 발생해야 한다.

다음 profile 단계에서 하지 않는 일:

- FedMatch/FedLGMatch/(FL)^2 구현을 시작하지 않는다.
...
- `main_server/src/services/federation/rounds/`
  ...
  - 진행: aggregation adapter의 공통 runtime validation은 `runtime_adapter.py`로 올렸다.
  - 진행: LoRA-classifier aggregate artifact ref 생성 규칙은 `artifact_refs.py`로 분리했다.
  - 진행: aggregation adapter 파일은 adapter family가 아니라 aggregation method 단위
    `fedavg.py`로 정리했고, method 파일 내부도 generic runtime/spec 구조로 접었다.
  - 진행: server-owned prototype artifact lifecycle은 `federation/prototypes`로 위치를
    명확히 했다.
  - 문제: 실제 artifact-ref materialization과 FedMatch/FedLGMatch server-side
...
- `methods/federated_ssl/training_algorithm_profiles.py`,
  ...
  - 남은 문제: Hydra local update profile과 Python mapping/default가 같은 실행 조합 의미를
    반복할 수 있다.
  - 다음 작업: profile source-of-truth 단계에서 Hydra/Python 중복 실행값 inventory를
    먼저 만들고, 각 Python module을 compatibility facade, typed parser/validator,
    테스트 fixture helper 중 하나로 좁힌다.
