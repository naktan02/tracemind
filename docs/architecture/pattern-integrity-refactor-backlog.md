...
- update-family FL SSL slice는 method 이름 파일로 증식시키지 않는다.
...
| main server payload adapter | generic shared-adapter runtime과 methods aggregation core 조합 | update family별 round runtime 파일 추가 금지 |
| aggregation | strategy wiring은 methods, artifact ref materialization은 server capability | main_server에 update family별 aggregation 구현 금지 |
| partitioned delta average | update family가 partition payload materialization과 평균 backend를 소유 | registry convention만 맞추는 `methods/federated/aggregation/partitioned_*` 얇은 폴더 금지 |
...
| FL SSL capability axes | `capabilities/plan.py`와 `capabilities/axes.py`가 공통 capability 이름을 소유 | tiny `local_ssl_policy.py`/`server_update_policy.py` 재분리 금지 |
| FedMatch recipe metadata | `fedmatch/descriptor.py`에서 recipe metadata를 직접 노출 | pass-through `fedmatch/recipe.py` 재도입 금지 |
| update-family FL SSL slice | family 실행 primitive는 `methods/adaptation/<family>/federated_ssl/` 소유 | `<method>_*.py` 파일 증식 금지 |
| Query SSL view preparation | `methods/adaptation/query_text_views/unlabeled_preparation.py`가 view surface별 row preparation을 소유 | scripts에 USB view builder/augmentation source 분기 재도입 금지 |
...
- `methods/federated_ssl/local_ssl_policy.py`, `methods/federated_ssl/server_update_policy.py`
  제거: 같은 capability plan 주변의 작은 policy name/normalizer라 `capabilities/axes.py`로
  합쳤다.
...
- `methods/federated/aggregation/partitioned_fedavg/` 제거: 실제 generic 산술을 소유하지
  않고 registry convention만 만족시키던 얇은 package라 제거했다. PEFT text encoder
...
1. `scripts/support/query_ssl_text_encoder/text_encoder_run_context.py`: 외부 framework context와 내부
   실행값이 섞여 보인다.
