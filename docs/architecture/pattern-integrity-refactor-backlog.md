...
- 완료: compatibility validator는 method descriptor와 recipe metadata,
  ...
  한 번에 본다.
- 완료: `fl_profile`/`experiment_profile` metadata는 실행값 source-of-truth가 아니라
  compose preset 이름표로 해석하고, 실제 compose된 method/local/round profile과
  다르면 bootstrap 전 compatibility validator에서 실패한다.
...
- `main_server/src/services/experiment_workspace/compiler/service.py`
  ...
  - 완료: catalog lookup(`catalog_lookup.py`), selection compile(`selection_compiler.py`),
    Hydra override utility(`hydra_overrides.py`)를 분리했다.
  - 완료 기준: `service.py`는 catalog 조회, selection compile, policy 실행, output payload
    조립을 orchestration하고 selection loop/override formatting 세부사항은 소유하지 않는다.
