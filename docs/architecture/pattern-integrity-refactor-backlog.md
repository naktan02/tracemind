...
- `methods/federated/aggregation/partitioned_fedavg/` 제거: 실제 generic 산술을 소유하지
  ...
  `methods/adaptation/text_classifier/aggregation/peft_encoder_partitioned_projection.py`가
  소유한다.

다음 변경 시 함께 볼 후보:

1. `scripts/experiments/query_lora_ssl/harness/common.py`: 외부 framework context와 내부
   실행값이 섞여 보인다.
2. `agent/src/services/wellbeing/child_support_service.py`: wellbeing orchestration 첫 화면이
   길다.
3. `methods/adaptation/lora_classifier/training_backend.py`: facade 이름을 유지할 만큼
   interface가 깊은지 삭제 테스트가 필요하다.
