...
- `scripts/experiments/prototype_analysis/prototype_strategy/projection.py`: projection,
  prototype point assembly, visual center 계산, figure drawing이 한 service에 모여 있다.
...
- `main_server/src/services/experiment_workspace/catalog/tracks.py`와
  `federated_runtime_sections.py`: 아직 길지만 대부분 declaration table이라 즉시 분리보다
  table/helper 구조 유지가 더 안전하다.
