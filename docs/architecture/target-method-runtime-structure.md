...
새 FL SSL 방법, classifier 기반 update, PEFT mechanism을
...
`scripts`가 `classifier`, `lora`, `fedmatch_agreement` 같은 method 또는
...
| trainable state / update family | client가 학습하고 server가 해석하는 공유 가능 상태 family | `linear_head`, `peft_text_encoder` |
| scorer family | local inference/training에서 score를 만드는 방식 | `classifier_logits` |
...
새 trainable state가 추가되면 `update_family_name` 또는
...
그 사실로 분기하지 않는다. 반대로 `linear_head`, `classifier_logits`는 embedding/vector 위에서 동작하는
...
shared/
  contracts/
    trainable_state/{linear_head,peft_text_encoder}.py
...
scripts/
  experiments/
    {fl_ssl,central_ssl}/{run,sweep,report}.py
...
1. `linear_head`는 작은 classification primitive다.
   ...
   scripts와 report는 head 구현 세부를 직접 알지 않는다.

2. `fedmatch_agreement` 같은 method-local objective는 generic config axis leaf로
...
5. `scripts`의 Interface는 config load, execution plan resolve, runtime context 구성,
  ...
  필요하면 runtime adapter executor path를 선언할 수 있다.
   ...
   `linear_head`, `peft_text_encoder` 같은
...
6. 중앙 SSL의 teacher는 독립 실행 stage나 별도 strategy axis가 아니다.
   ...
   PEFT checkpoint, EMA teacher, external pseudo-label source는 별도
...
- `fixed_classifier_seed`는 active 실행 축이 아니다. classifier/PEFT 같은
...
새 FL SSL method나 update family를 추가할 때 변경 위치가 아래로 닫히면 성공이다.

```text
methods/federated_ssl/<method>/*
conf/strategy_axes/fssl_method/<method>.yaml
conf/strategy_axes/ssl_objective/local_update_profile/<profile>.yaml
conf/strategy_axes/model_architecture/update_family/<family>.yaml  # runtime callable까지 구현될 때 추가
tests/*
```
