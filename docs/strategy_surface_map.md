...
- `report.track=fl_ssl_main_comparison`은 중앙 SSL control과 섞이지 않는
  FL main comparison report schema다. 현재 report shape는
  `run_federated_simulation.yaml` 안의 entrypoint-local section이다.
