...
| 새 trainable/update family 추가 | prototype pack update, projection head, full encoder, 다른 PEFT classifier family 추가 | payload shape가 바뀌면 `shared`, 계산 core는 `methods`, 실행 조합은 `conf` |
...
3. state/update payload가 달라지면 "새 trainable/update family 추가"다.
...
1. 바뀌는 축을 확정한다.
   - `training_backend`, `example_generation_backend`, `scorer_backend`,
     ...
     `prototype_builder`, `update_family`, `payload_adapter_kind` 중 무엇인지
     먼저 적는다.
2. contract 변경이 필요한지 판별한다.
   - 새 payload shape가 없으면 `shared/src/contracts/`는 건드리지 않는다.
   - 새 payload family면 `shared/src/contracts/adapter_contract_families/`부터 본다.
3. concrete 구현을 추가한다.
...
예: 같은 `peft_text_encoder` update family 안에서 다른 FedAvg weighting backend 추가
...
## 4. 새 trainable/update family 추가
...
6. shared payload registry와 aggregation backend만으로 generic payload adapter runtime이
...
4. `methods/adaptation/<family>/federated_ssl/`에도 `<method>_*.py`를 기본값으로
   추가하지 않는다. update-family runtime 쪽은 partitioned loop, server update
   resolver처럼 실행 primitive 이름을 쓰고, method 의미는 descriptor/local objective가
   소유한다.
