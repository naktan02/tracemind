...
| 새 trainable/update family 추가 | prototype pack update, projection head, full encoder, 다른 PEFT encoder family 추가 | payload shape가 바뀌면 `shared`, 계산 core는 `methods`, 실행 조합은 `conf` |
