...
열렸고, method-owned tensor local objective core까지 추가했다. 실제 trainer
partition wiring/server-step runtime은 다음 구현 단계다. 각 method의 source of truth는
...
- `methods/federated_ssl/fedmatch/`
  ...
  - 현재 status는 `tensor_local_objective_core_v1`이다. 원본 설정값, confidence
    filter, agreement pseudo-label vote, helper top-k selection, LoRA-classifier
    sigma/psi 매핑, supervised/unsupervised FedMatch tensor loss는 methods core에
    고정했다.
  - LoRA trainer sigma/psi optimizer split, helper prediction exchange, sparse
    S2C/C2S sync, server step runtime은 아직 실행되지 않는다.
...
원본 core/config snapshot, tensor local objective core다.
