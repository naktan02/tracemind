...
- query-domain 적응 단계의 `central PEFT text encoder` checkpoint 전체를 설명하는
  문서는 아니다.
...
`shared adapter`는 linear head, PEFT text encoder 같은
...
- 이것은 특정 category anchor 쪽으로 점을 직접 끌어당기는 translation이 아니다.
- 공간 전체에 공통으로 적용되는 차원별 rescaling이다.
- 따라서 "특정 anchor로 직접 이동"이라기보다,
  "accepted example 분포에 맞게 각 축을 조금 늘리고 줄여서
   이후 임베딩 공간의 상대 위치가 달라지게 만드는 방식"에 가깝다.
...
4. 이 family는 시스템 translation용 shared head를 뜻하며, 논문 트랙의
   frozen backbone + PEFT text encoder checkpoint 전체와 동일한 개념은 아니다.
...
3. classifier score로 pseudo-label을 고른다.
