...
`query_ssl.p_cutoff`, `query_ssl.temperature`, `query_ssl.hard_label`,
`query_ssl.lambda_u` 같은 값은 TrainingTask contract가 소유하는 기본값이 아니라
FixMatch 같은 SSL method가 소유하는 resolved method parameter다. source of truth는
`conf/strategy_axes/ssl_objective/consistency_method/*.yaml`과
`methods/ssl/algorithms/<method>/`에 있다. server가 agent에 task를 발행할 때는
agent가 task payload만 보고 실행할 수 있도록 해당 값을 `query_ssl.*` extra로
전달할 수 있다.
