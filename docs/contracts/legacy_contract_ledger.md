...
| `adapter_kind=lora_classifier`, `lora_classifier_update`, `lora_classifier_delta.v1` | `adapter_kind=peft_classifier`, `peft_classifier_update`, `peft_classifier_delta.v2` | v2: `shared/src/contracts/adapter_contract_families/peft_classifier.py`, legacy readers: report/materialization owner files, `docs/contracts/lora_classifier_v1_terminology_audit.md` | report verifier, result-index/dashboard, 기존 run artifact | shared v1 parser/factory와 golden fixture는 제거됐다. 기본 FL simulation config와 v2 producer는 `peft_classifier`를 쓰고, legacy 값은 old artifact/report reader가 자기 경계에서만 읽은 뒤 canonical 표면으로 정규화한다. | old-run report/artifact/dashboard ingest가 v2 canonical 산출물만 필요하다고 확인되고, 남은 old-reader fallback을 제거할 때 |
