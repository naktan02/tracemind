...
| `adapter_kind=lora_classifier`, `lora_classifier_update`, `lora_classifier_delta.v1` | 후보: `text_classifier_peft_encoder`, `text_classifier_peft_update`, `text_classifier_peft_delta.v2` | `shared/src/contracts/adapter_contract_families/lora_classifier.py`, `docs/contracts/lora_classifier_v1_terminology_audit.md` | shared contract, FL simulation/runtime configs, report verifier, 기존 run artifact | adapter family 식별자와 payload format은 cross-boundary contract라 폴더 이동과 동시에 바꾸면 producer/consumer drift가 큼 | producer, consumer, verifier, dashboard/report fixture가 v2 이름을 동시에 지원하고 v1 legacy ledger가 별도 compatibility path로 격리됐을 때 |
