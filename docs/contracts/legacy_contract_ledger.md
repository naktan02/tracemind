...
| Central PEFT report `manifest.backbone.lora` key | `manifest.backbone.peft_adapter_config` | `methods/adaptation/peft_text_encoder/training/modeling.py`, `scripts/experiments/result_index/report_loader.py` | old central PEFT report ingest | LoRA mechanism config를 PEFT adapter config surface로 승격하면서 기존 report를 계속 읽기 위함 | central PEFT report cache가 새 key로 재생성되면 `report_loader` fallback 삭제 |
| Dashboard bundle `central_lora_*`, `lora_*`, `adapter_family_name` run/filter fields | `central_peft_*`, `peft_adapter_*`, `payload_adapter_kind` | `apps/experiment_dashboard/src/app.js`의 bundle load normalizer | committed dashboard cache | `apps/experiment_dashboard/data/**`는 기존 자료 cache라 직접 수정하지 않고, UI reader가 current field로 정규화한다. 새 result-index export는 `peft_adapter_*`와 `payload_adapter_kind`만 생산한다. | dashboard cache가 새 result-index export로 재생성되면 app load normalizer fallback 삭제 |
