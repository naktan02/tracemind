...
| `adapter_kind=lora_classifier`, `lora_classifier_update`, `lora_classifier_delta.v1` | `adapter_kind=peft_classifier`, `peft_classifier_update`, `peft_classifier_delta.v2` | v1: `shared/src/contracts/adapter_contract_families/lora_classifier.py`, v2: `shared/src/contracts/adapter_contract_families/peft_classifier.py`, `docs/contracts/lora_classifier_v1_terminology_audit.md` | shared contract, report verifier, 기존 run artifact | adapter family 식별자와 payload format은 cross-boundary contract라 producer/consumer drift를 막기 위해 v1 reader를 유지한다. 기본 FL simulation config와 v2 producer는 `peft_classifier`를 쓰고, v1은 legacy artifact/report reader로 남긴다 | verifier, dashboard/report fixture, old-run loader가 v2 primary와 v1 legacy fallback을 모두 검증하고, 새 산출물이 v2 single-write로 충분히 누적됐을 때 |
| `scripts/experiments/fl_ssl/federated_simulation/**`와 `scripts/runtime_adapters/federated_agent/**` 안의 `lora_classifier` key, verifier flag, legacy evaluator name, bridge alias | `round_runtime.peft_classifier`, `peft_classifier_eval`, `simulation_peft_classifier_state_ref`, `expect_peft_classifier_aggregate_snapshot`, canonical implementation owner는 `methods/adaptation/peft_text_classifier/**` | `scripts/experiments/fl_ssl/federated_simulation/models.py`, `config_request.py`, `flow/bootstrap.py`, `io/report_verification*.py`, `adapters/evaluation.py`, `adapters/server_step_execution.py`, `scripts/runtime_adapters/federated_agent/base_state_materialization.py`, `local_training.py`, `scripts/experiments/result_index/**`, `methods/adaptation/peft_text_classifier/aggregation/peft_encoder_*_projection.py`, `tests/architecture/test_layer_dependencies.py` | report verifier CLI/API, resume/report artifacts, runtime bridge tests, result index/dashboard | active Hydra root config와 FedMatch recipe에서는 legacy `lora_classifier` 실행 leaf를 제거했다. 기존 run artifact가 `lora_classifier_eval`, `expect_lora_classifier_aggregate_snapshot`, `simulation_lora_classifier_state_ref`, `compute_lora_classifier_*` registry metadata를 읽을 수 있으므로 reader fallback은 유지한다. | 새 FL 산출물이 v2 single-write로 충분히 누적되고 기존 artifact/verifier compatibility window가 종료되어 architecture allowlist를 축소할 때 |
