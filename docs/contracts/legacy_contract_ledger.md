...
| `scripts/experiments/fl_ssl/federated_simulation/**`와 `scripts/runtime_adapters/federated_agent/**` 안의 `lora_classifier` key, verifier flag, legacy evaluator name, bridge alias | `round_runtime.peft_classifier`, `peft_classifier_eval`, `simulation_peft_classifier_state_ref`, `expect_peft_classifier_aggregate_snapshot`, canonical implementation owner는 `methods/adaptation/text_classifier/peft_encoder/**` | `scripts/experiments/fl_ssl/federated_simulation/models.py`, `config_request.py`, `flow/bootstrap.py`, `io/report_verification*.py`, `adapters/evaluation.py`, `adapters/server_step_execution.py`, `scripts/runtime_adapters/federated_agent/base_state_materialization.py`, `local_training.py`, `tests/architecture/test_layer_dependencies.py` | FL simulation Hydra config, report verifier CLI/API, resume/report artifacts, runtime bridge tests | 기존 run artifact와 Hydra override가 `round_runtime.lora_classifier`, `lora_classifier_eval`, `expect_lora_classifier_aggregate_snapshot`, `simulation_lora_classifier_state_ref`를 읽는다. 현재 기본 Hydra 실행과 운영 verifier manifest는 `peft_classifier`/`peft_classifier_eval`/`expect_peft_classifier_aggregate_snapshot`을 쓰고, legacy 이름은 compatibility 표면으로만 남긴다 | dashboard/result index와 old-run artifact loader까지 v2 primary + v1 fallback 검증이 끝나고 architecture allowlist를 축소할 때 |
