...
| Central PEFT report `manifest.backbone.lora` key | `manifest.backbone.peft_adapter_config` | `methods/adaptation/peft_text_encoder/training/modeling.py`, `scripts/experiments/result_index/report_loader.py` | 제거 완료 | LoRA mechanism config를 PEFT adapter config surface로 승격했고 current result-index reader는 새 key만 읽는다. | 완료 |
