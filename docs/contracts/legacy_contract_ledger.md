...
| `LOCAL_SSL_POLICIES_FROM_QUERY_SSL` | `QUERY_SSL_LOCAL_OBJECTIVE_POLICY_NAMES`, `is_query_ssl_local_objective_policy()` | `methods/federated_ssl/capabilities/axes.py` | legacy import 경로가 남아 있을 수 있는 runtime adapter/test | 과거 이름을 읽는 호출부를 한 compatibility window 동안 깨지 않기 위함. active runtime은 predicate를 사용한다. | repo/API 소비자 grep에서 legacy constant import가 비고, downstream migration 공지가 끝나면 alias 삭제 |
