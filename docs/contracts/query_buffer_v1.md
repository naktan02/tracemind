...
3. 이후 `LoRA + classifier` 적응 또는 FL translation으로 넘길 후보를 만든다.
...
1. LoRA 적응과 weak/strong augmentation에는 raw text가 필요하다.
2. 나중에 backbone/LoRA가 바뀌면 과거 embedding은 stale해질 수 있다.
...
4. LoRA 적응에 필요한 원문이 retention window 안에서 보존된다.
