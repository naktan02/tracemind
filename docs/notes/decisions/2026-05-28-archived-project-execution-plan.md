...
- FL SSL에서 `PEFT encoder classifier`를 공유 가능한 trainable state로 승격할 때의
  target canonical update family 이름은 `peft_text_encoder`로 둔다.
  `lora_classifier` shared contract와 report/result reader fallback은 제거된 상태를
  유지한다. LoRA는 PEFT adapter mechanism이고, classifier head는 현재
  `methods/classification/linear_head` primitive로 구현된 head/scorer이므로 두
  개념을 `classifier_head` 옵션이나 bare `lora` family로 숨기지 않는다.
- `peft_text_encoder`의 1차 범위는 FL simulation research path이고, live
  `agent`/`main_server` runtime translation은 2차 범위다. 현행 실행 field는
  `payload_adapter_kind=peft_classifier`와
  `update_family_name=peft_text_encoder`를 쓴다.
- PEFT text encoder 비교의 고정 조건은 `mxbai_encoder`, tokenizer, LoRA
...
- 현재 FL SSL 기본 실행 조합은 descriptor 없는
  `manual + FixMatch + PEFT text encoder + FedAvg`다.
...
```text
Query Buffer -> Selection -> Accepted Rows -> PEFT text encoder + head -> Central Evaluation
```
...
- method selection: 현재 기본 baseline은 v1 실행 field 기준
  ...
  `round_runtime.update_family_name=peft_text_encoder`,
...
- 현재 1순위 translation 후보는 target 기준 `peft_text_encoder` update family다.
  v1 `lora_classifier` contract/report compatibility는 제거된 상태를 유지한다.
- `peft_text_encoder` state/update payload는 PEFT adapter state와 linear classifier
...
- 실제 PEFT executor 기준 `FixMatch + PEFT text encoder + FedAvg` 1-round smoke는
  완료했다. report metadata는 `query_ssl_method.algorithm_name=fixmatch`로
  `methods/ssl/algorithms/*`가 실제 local objective를 소유함을 남긴다.
- PEFT adapter/head delta artifact 경로는 `agent-local://` ref를 server-owned
...
1. FedMatch reduced run 전에 PEFT text encoder simulation 병목을 줄인다.
...
3. runtime resource cache seam은 `methods.common` protocol과 simulation run-scoped
   in-memory cache로 열었다. PEFT text encoder model builder는 cache가 있으면
   tokenizer와 frozen backbone base를 재사용하고, client별 PEFT adapter/head state를 별도 model
...
4. helper snapshot별 materialized helper model cache를 추가했다. 같은 run에서 동일
   helper snapshot이 다시 선택되면 PEFT text encoder model 복원과 parameter load를
   재사용한다.
5. client-local pseudo-label quality 진단은 `diagnostic_view.max_rows=512` 기본값의
   ...
   method-owned PEFT text encoder 경로가 같이 사용한다.
...
8. FedMatch처럼 partitioned server update policy가 partition별 material만 소비하는
   ...
   primary PEFT adapter/head delta artifact도 생략해 shared update payload와 중복 artifact 저장을
...
11. 같은 split/seed/budget에서 현행
   `FixMatch + PEFT text encoder + FedAvg` manual baseline과 FedMatch method-owned slice를
   비교 가능한 reduced report로 맞춘다. target 구조에서는 이 manual baseline을
   `update_family=peft_text_encoder`로 표현한다.
...
13. full ablation, full `client_count=1..10` sweep, full-budget main run은 후보와
   ...
   stress 확인으로 남긴다.
14. winner를 target 기준 `peft_text_encoder` update family 또는 현실적인 fallback
...
3. FL 범위를 `peft_text_encoder` update family에서 PEFT adapter, head/scorer,
