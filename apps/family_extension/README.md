...
- `src/collector/content.ts`
  - `input`, `textarea` surface를 감지한다.
  - rich editor와 `contenteditable` surface는 일부 editor의 IME 처리와 충돌해
    실제 입력을 훼손할 수 있어 기본 수집 대상에서 제외한다.
...
- `collector-fixture.html`
  ...
  - content script 주입 확인용으로 `input`, `textarea`, `contenteditable`을 제공한다.
    현재 `contenteditable`은 안전을 위해 수집되지 않아야 한다.
