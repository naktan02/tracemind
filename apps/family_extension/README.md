...
- `src/extension/background.ts`
  ...
  - proactive prompt는 content tab으로 전달하되, 사용자가 닫으면
    `chrome.storage.local`에 짧은 쿨다운을 저장해 바로 다시 띄우지 않는다.
  - proactive prompt가 포함한 `conversation_id`를 content tab에 같이 전달해
    아이의 첫 답변이 agent-local 대화 히스토리로 이어지게 한다.
