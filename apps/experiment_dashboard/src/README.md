...
2. `central_ssl/`: 중앙 SSL control 화면 상태, 필터, selector, 페이지 UI.
3. `fl_ssl/`: FL SSL main comparison 화면 상태, 필터, selector, 페이지 UI.
...
- `central_ssl/logic`과 `fl_ssl/logic`은 각 track의 run 해석과 필터 축을 소유한다.
- `central_ssl/ui`와 `fl_ssl/ui`는 HTML fragment 조합만 맡고, bundle schema 의미를
  새로 정의하지 않는다.
