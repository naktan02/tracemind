...
왼쪽 track navigation은 `Central SSL`, `Supervised`, `전체`, `FL SSL`을
제공한다. 현재 기본 화면은 `Central SSL` track이며 method, split, eval set을 고른 뒤
여러 run과 여러 metric을
동시에 선택해 bar/line chart로 비교할 수 있다. per-class metric과 confusion matrix,
projection image는 첫 번째 선택 run을 상세 대상으로 사용한다.

`Supervised`는 중앙 지도학습 baseline을 필터링한 view이고, `전체`는 중앙 SSL/지도학습
과 `FL SSL` 결과를 동시에 비교할 수 있는 통합 view다. `전체`는 표/그래프 두 탭만
노출한다. `FL SSL` track은 중앙 SSL
ranking과 섞지 않는 별도 표면이다. result index export는 FL report를 dashboard
전용 view-model로 평탄화한다.
